    ...
    existingNodes.forEach(node => {
      ...
      setTimeout(() => {
        ...
        function createNodeElement(nodeInfo) {
          ...
          // 带递归动画的树节点创建函数
function createTreeNodesWithAnimation() {
    if (!treeContainer) return;
    ...
    setTimeout(() => {
        ...
        // 计算容器内边距，确保节点不会超出边界
        const drawingPadding = 15; // 绘制区域内边距
        const drawingWidth = actualContentWidth - 2 * drawingPadding; // 实际绘制区域宽度
        
        // 分配叶子节点位置 - 确保节点不会重合或贴边
        const leafPositions = new Map();
        leafNodes.forEach((leaf, index) => {
            let position;
            if (leafNodes.length === 1) {
        ...
        // 递归动画创建节点函数
        function createNodeElementWithAnimation(nodeInfo, delay = 0) {
            const { node, start, end, level, position } = nodeInfo;
        ...
        function animateNodeCreation(node, start, end, delay = 0) {
            ...
            if (nodeInfo) {
                createNodeElementWithAnimation(nodeInfo, currentDelay + 200);
            }
        ...
        // Add connecting lines after all animations complete
        setTimeout(() => {
            addConnectingLines();
        }, totalDelay + 1000);
...
function initEventListeners() {
    ...
    updateBtn.addEventListener('click', function() {
        ...
        try {
            ...
            // 在构建树后调整容器大小
            adjustContainerSize();
            // 触发递归动画展示树            drawTreeWithAnimation();
        } catch (error) {
